    ...
    public class TreeViewItemWidthMatcherProperty : BaseAttachedProperty<TreeViewItemWidthMatcherProperty, bool>
    {
        public override void OnValueChanged(DependencyObject sender, DependencyPropertyChangedEventArgs e)
        {
            ...
            onLoaded = (s, ee) =>
            {
                ...
                foreach (var child in panel.Children)
                {
                    ...
                    if (!(child is TextEntryControl) && !(child is PasswordEntryControl) && !(child is HierarchyItemSelectionControl))
                        continue;

                    // Get the label from the text entry or password entry

                    var label = child is TextEntryControl ? (child as TextEntryControl).Label : ((child is PasswordEntryControl ? (child as PasswordEntryControl).Label
                        : (child as HierarchyItemSelectionControl).Label));
        ...
        private void SetWidths(Panel panel)
        {
            ...
            foreach (var child in panel.Children)
            {
                ...
                // Get the label from the text entry, password entry, or Hierarchy item selection control
